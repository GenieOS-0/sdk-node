...
- Links deep surface: `links.get` / `links.update` / `links.analytics`.
- QR designs: `qr.create` / `qr.update` / `qr.render` (svg/png/webp/
  png-print/pdf, optional `saveToAssets`).
- Widened `links.create` — password, expiry, scheduled go-live, route
  rules (incl. city), Spark+ custom domains.
- Webhook events: `link.clicked`, `link.created`, `link.updated`,
  `link.archived`, `link.abuse_flagged`, `link.disabled_for_abuse`,
  `link.reinstated` (Glow+).
